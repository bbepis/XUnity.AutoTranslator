...
namespace XUnity.Common.Utilities
{
            ...
            get
            {
                if (_current == null)
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        XuaLogger.AutoTranslator.Warn( "[UnityInput] Failed to detect available input systems - " + ex);
                    }
                }
                return _current;
            }
    ...
    internal class NewInputSystem : IInputSystem
    {
        ...
        private static ButtonControl GetControl(KeyCode key, bool silent = false)
        {
            ...
            if (!silent)
                XuaLogger.AutoTranslator.Warn( $"[{nameof(NewInputSystem)}] Unsupported key: {key}" );
